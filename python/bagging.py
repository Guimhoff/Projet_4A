...
class Bagging(BaseEstimator, ClassifierMixin):
    ...
    # Constructor method
    def __init__(self, tree_count=50, single_tree_data_ratio=.6, max_depth=999):
    ...
    def fit(self, X, y):
        ...
        for _ in range(self.tree_count):
            n_X = None
            n_y = None
            if self.single_tree_data_ratio == 1:
                n_X = X
                n_y = y
            else:
...
def example():
    ...
    test_bagging = Bagging().fit(X_train, y_train)
    ...
    print("Accuracy of the model (with training data) : ", test_bagging.score(X_train, y_train))
    em.evaluateModel(test_bagging, X_test, y_test, False)

    print(" === Cross validation === ")
    em.k_fold(Bagging, iris['data'], iris['target'])
    print(" === Stratified cross validation === ")
    em.stratified_k_fold(Bagging, iris['data'], iris['target'])
